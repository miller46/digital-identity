...
web3.version.getNetwork(function(networkError, version) {
    ...
    Web3Utility.loadContract(Config.personaRegistryContract, Config.personaRegistryAddress, function(contractError, contract) {
        if (contractError) {
            ...
            Web3Utility.callContractFunction(contract, Config.personaRegistryAddress, 'getMyPersona', [], function(functionError, result) {
                if (functionError) {
...
function initClickListeners() {
    $('#create-button').click(function() {
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            if (value) {
                nonEmptyFields.push({field: web3.fromAscii(field, 32), value: web3.fromAscii(value, 32)});
            }
        }
        ...
        async.each(nonEmptyFields, function(item, callback) {
            ...
            Crypto.encrypt(userAccount.publicKey, item.value, function(value) {
            ...
            if (typeof error !== 'undefined') {
                ...
                Web3Utility.callContractFunction(registryContract, Config.personaRegistryAddress, 'addPersona', [nonEmptyFields, ipfsPointers], function (functionError, result) {
                    if (functionError) {
                        console.log(functionError);
                    } else {
                        console.log(result);
